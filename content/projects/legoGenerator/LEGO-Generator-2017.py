...
for col in colorMap:
    cmds.button(col, label="", bgc=colorMap[col], w=20, h=20, c=("colorChange('" + col + "')"))
cmds.setParent("..")
cmds.setParent("..")
...
def createSquarePolys():
    ...
    if(False == noNubs):
        for i in range(width):
            for j in range(depth):
                ...
                base = cmds.polyCBoolOp(base[0], nub[0], op=1, ch=False)
    ...
    base = cmds.polyCBoolOp(base[0], tmp[0], op=2, ch=False)
...
def createRoundPolys():
    ...
    base = cmds.polyCBoolOp(base[0], cyldr[0], op=1, caching=False, ch=False)
    ##right
    if(isSpacerEnd):
        ...
        cmds.move((sizeX * 0.5), moveX=True, a=True)
        base = cmds.polyCBoolOp(base[0], spacer[0], op=1, ch=False)
        cmds.select(axel[0])
        cmds.rotate(90, rotateZ=True, r=True)
        base = cmds.polyCBoolOp(base[0], axel[0], op=2, ch=False)
    else:
        cyldr = cmds.polyCylinder(r=sizeY*0.5, h=sizeZ)
        cmds.rotate(90, rotateX=True, a=True)
        cmds.move((sizeX * 0.5), moveX=True, a=True)
    ...
    if(bend > 1):
        ...
        cmds.rotate(90, rotateX=True, a=True)
        cmds.move((sizeX * 0.5), moveX=True, a=True)
        arm = cmds.polyCBoolOp(arm[0], cyldr[0], op=1, caching=False, ch=False)
        #add holes
        print(axelPos)
        ...
        if(bend == 3):
            ...
            cntr = cmds.polyCBoolOp(cntr[0], tmp[0], op=1, ch=False)
            arm2 = cmds.polyCBoolOp(arm2[0], cntr[0], op=2, ch=False)
            #combine
            arm = cmds.polyCBoolOp(arm2[0], arm[0], op=1, ch=False)
            #hole
        ...
        base = cmds.polyCBoolOp(base[0], arm[0], op=1, ch=False)
...
def createTirePolys(scl=1):
    ...
    tire = cmds.polyCBoolOp(tire[0], tmp[0], op=3, ch=False)
    #cylinder for tread
    ...
    tread = cmds.polyCBoolOp(tread[0], tmp[0], op=1, ch=False)
    tread = cmds.polyCBoolOp(tread[0], tread2[0], op=1, ch=False)
    #round it out
    tmp = cmds.polySphere(r=3.5, sy=13, sx=36)
    cmds.scale(0.95, scaleY=True)
    tread = cmds.polyCBoolOp(tread[0], tmp[0], op=3, ch=False)
    cmds.polyMergeVertex(d=0.05, ch=False)
    #combine
    tire = cmds.polyCBoolOp(tire[0], tread[0], op=1, ch=False)
    ...
    for i in range(2):
        ...
        tmp = cmds.polyCBoolOp(tmp[0], tmp1[0], op=2, ch=False)
        cmds.move(1.4 - i*2.8, moveY=True, a=True)
        tire = cmds.polyCBoolOp(tire[0], tmp[0], op=2, ch=False)
    #harden edges
...
def createHubPolys(scl=1):
    ...
    cmds.select(tmp[0])
    cmds.delete(ch=True)
    hub = cmds.polyCBoolOp(hub[0], tmp[0], op=2, ch=False)
    #internal spokes
    ...
    hub = cmds.polyCBoolOp(hub[0], tmp[0], op=1, ch=False)
...
def createAxelPolys(fromUI=True, length=5):
    ...
    pill = cmds.polyCBoolOp(pill[0], tmp[0], op=1, ch=False)
    #bottom
    tmp = cmds.polySphere(r=0.25)
    cmds.scale(0.5, scaleY=True, a=True)
    cmds.move(-(sizeY-0.25)*0.5, moveY=True, a=True)
    ...
    axel = cmds.polyCBoolOp(axel[0], pill[0], op=3, ch=False)
...
def createGearPolys():
    ...
    if(teeth > 8):
        ...
        gear = cmds.polyCBoolOp(gear[0], tmp[0], op=2, ch=False)
        tmp = cmds.polyCylinder(r=gearRad-toothLen*0.5 - 0.1, h=0.2, sx=teeth*2)
        cmds.move(-0.2, moveY=True, a=True)
        gear = cmds.polyCBoolOp(gear[0], tmp[0], op=2, ch=False)
    #add axel hole
    tmp = cmds.polyCylinder(r=0.3, h=0.8, sx=teeth*2)
    gear = cmds.polyCBoolOp(gear[0], tmp[0], op=1, ch=False)
    tmp = createAxelPolys(False)
    gear = cmds.polyCBoolOp(gear[0], tmp[0], op=2, ch=False)
...
def createConnectorPolys():
    ...
    for i in range(2):
        ...
        if(type == "Axel"):
            ...
            pin = cmds.polyCBoolOp(pin[0], tmp[0], op=2, ch=False)
        #female axel on one side (spacer)
            ...
            pin = cmds.polyCBoolOp(pin[0], tmp[0], op=1, ch=False)
        ...
        #one side is default connector (or half or double)
            ...
            tmp = cmds.polyCBoolOp(tmp[0], tmp2[0], op=2, ch=False)
    ...
    connector = cmds.polyCBoolOp(connector[0], connector[1], op=1, ch=False)
    cmds.delete(ch=True)
    return connector
...
def createSpacerPolys(fromUI=True, width=1, rimSize=0.35):
    ...
    if(size == 0.4):
        ...
        tmp = cmds.polyCBoolOp(tmp[0], tmp2[0], op=1, ch=False)
    spacer = cmds.polyCBoolOp(spacer[0], tmp[0], op=2, ch=False)
...
def addHoles(base, numHoles, offsetY=0.0, offsetX=0.0, axelPos=[], isHalfDepth=False):
    for i in range(numHoles):
        ...
        base = cmds.polyCBoolOp(base[0], hole[0], op=2, caching=False, ch=False)
    return base
...
def getHoleShape(isHalfDepth=False):
    ...
    edge = cmds.polyCylinder(r=0.31, h=0.1)
    cmds.move(width*0.5 - 0.04, moveY=True, a=True)
    hole = cmds.polyCBoolOp(hole[0], edge[0], op=1, caching=False, ch=False)
